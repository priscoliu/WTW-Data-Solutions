...
### 3. Building — Cell by Cell
- Follow the standard notebook structure:
  1. **Cell 1**: Setup & Configuration (imports, helper functions)
  2. **Cell 2**: Load Bronze Data (source tables + stream-level filters)
  3. **Cell 3**: Transformation Logic (calculated columns per stream)
  4. **Cell 4**: Union & Unification (union streams, rename columns, cleanse, safe cast)
  5. **Cell 5**: Reference Joins (load ref tables, apply joins)
  6. **Cell 6**: Final Select & Write to Silver
