let
  ...
  #"Choose columns" = Table.SelectColumns(#"Added REINSURANCE DESCRIPTION", {"INVOICE NO", "POLICY DEPT", "INSURER NAME", "INSURER COUNTRY", "CLIENT NAME", "RISK DESCRIPTION", "INVOICE DATE", "EFFECTIVE DATE", "EXPIRY DATE", "ACCOUNT HANDLER", "Transaction Type", "Policy Description", "Party ID", "DUNS Number", "Revenue Country", "FINAL DATE", "Sub Product Class", "GLOBs", "GLOBS SPLIT P&C", "MAPPED_INSURER", "LLOYDS", "PREMIUM (USD)", "BROKERAGE (USD)", "SYSTEM ID", "CLIENTID", "BUSINESS TYPE", "DATA SOURCE", "REINSURANCE DESCRIPTION", "Lloyd's Asia or Lloyd's London"}),
  #"Renamed columns 1" = Table.RenameColumns(#"Choose columns", {{"INVOICE NO", "InvoicePolicyNumber"}, {"POLICY DEPT", "Department"}, {"INSURER NAME", "InsurerName"}, {"INSURER COUNTRY", "InsurerCountry"}, {"CLIENT NAME", "ClientName"}, {"RISK DESCRIPTION", "SystemProductId"}, {"INVOICE DATE", "InvoiceDate"}, {"EFFECTIVE DATE", "InceptionDate"}, {"EXPIRY DATE", "ExpiryDate"}, {"ACCOUNT HANDLER", "AccountHandler"}, {"Transaction Type", "TransactionType"}, {"Policy Description", "PolicyDescription"}, {"Party ID", "PartyIdWtw"}, {"DUNS Number", "DunsNumber"}, {"Revenue Country", "RevenueCountry"}, {"FINAL DATE", "FinalDate"}, {"Sub Product Class", "SubProductClass"}, {"GLOBs", "Globs"}, {"GLOBS SPLIT P&C", "GlobsSplitPnc"}, {"MAPPED_INSURER", "InsurerMapping"}, {"LLOYDS", "Lloyds"}, {"PREMIUM (USD)", "PremiumUsd"}, {"BROKERAGE (USD)", "BrokerageUsd"}, {"SYSTEM ID", "SystemId"}, {"CLIENTID", "ClientIdWtw"}, {"BUSINESS TYPE", "BusinessType"}, {"DATA SOURCE", "DataSource"}, {"REINSURANCE DESCRIPTION", "ReinsuranceDescription"}, {"Lloyd's Asia or Lloyd's London", "LloydsAsiaOrLondon"}})
in
  #"Renamed columns 1"
